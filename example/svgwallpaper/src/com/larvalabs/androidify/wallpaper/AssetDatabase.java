...
public class AssetDatabase {
    ...
    public SVG getSVGForAsset(String path, String name, String suffix, Integer searchColor, Integer replaceColor) {
        ...
        try {
            if (searchColor == null) {
                return new SVGBuilder().readFromAsset(assetManager, file).build();
//                return SVGParser.getSVGFromAsset(assetManager, file);
            } else {
                return new SVGBuilder().readFromAsset(assetManager, file).setColorSwap(searchColor, replaceColor).build();
//                return SVGParser.getSVGFromAsset(assetManager, file, searchColor, replaceColor);
    ...
    public SVG getSVGForResource(int resource, Integer searchColor, Integer replaceColor) {
        try {
            if (searchColor == null) {
                return new SVGBuilder().readFromResource(resources, resource).build();
//                return SVGParser.getSVGFromResource(resources, resource);
            } else {
                return new SVGBuilder().readFromResource(resources, resource).setColorSwap(searchColor, replaceColor).build();
